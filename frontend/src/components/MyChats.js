...
const MyChats = () => {
  ...
  const fetchChats = async () => {
    try {
      ...
      const lastSeenTime =
        JSON.parse(localStorage.getItem("lastSeenTime")) ||
        new Date(0).toISOString(); // fallback to old time
      const newNotifications = [];

      data.forEach((chat) => {
        if (
          chat.latestMessage &&
          new Date(chat.latestMessage.updatedAt) > new Date(lastSeenTime)
        ) {
            ...
            {chats
              ...
              .sort((a, b) => {
                const aTime = new Date(
                  a.latestMessage?.updatedAt || a.updatedAt
                ).getTime();
                const bTime = new Date(
                  b.latestMessage?.updatedAt || b.updatedAt
                ).getTime();
                return bTime - aTime;
              })
              .map((chat) => {
                      ...
                      <Box overflow="hidden">
                        ...
                        {chat.latestMessage && loggedUser && (
                          <Text fontSize="sm" noOfLines={1} opacity={0.8}>
                            <b>
                              {chat.latestMessage.sender?.name?.toLowerCase() ===
